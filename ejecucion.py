...
def main():
    ...
    if os.path.exists(process[0]):
        ...
        if params.get("archivo") + "_{}".format(datetime.now().strftime('%Y-%m-%d')) + ".csv" in archivos_csv:
            steps = [Transformations2.ejecutar(params), Controls(params, "test"), InferenceStep().ejecutar(params)] #RunTest.ejecutar(params, "test")]
    ...
    if os.path.exists(process[1]):
        ...
        archivos_csv = [archivo for archivo in archivos if archivo.endswith('.csv')]
        if params.get("archivo") + "_{}".format(datetime.now().strftime('%Y-%m-%d')) + ".csv" in archivos_csv:
            steps = [transformations_1.ejecutar(params), Controls(params, "train"), ModelTrain.train(params)]#RunTest.ejecutar(params, "train"), ]
